...
def relay_on(pin):
    ...
    GPIO.output(pin, GPIO.LOW) #set the relay pin to low (0V) to activate the relay

# Relay off
def relay_off(pin):
    print("Deactivating relay")
    GPIO.output(pin, GPIO.HIGH) #set the relay pin to high (3.3V) to deactivate the relay
      

def main():
    # Set up class
    gyro = 500      # 250, 500, 1000, 2000 [deg/s]    ; for rocket we want 2000 deg/s  
    acc = 8         # 2, 4, 8, 16 [g]                 ; for rocket we want +/- 16 g    
    tau = 1
    ...
    # Run for TBD 
    startTime = time.time()
    runningTime = 100 # time in seconds (should be around 15 minutes) secounds
    
    while(time.time() < (startTime + runningTime)):
